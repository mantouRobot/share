...
namespace frontier_exploration
{
    ...
    void BoundedExploreLayer::onInitialize(){
        ...
        //for gmapping use, explore_clear_space is false
        if(explore_clear_space){
            default_value_ = NO_INFORMATION;
        }else{
            default_value_ = FREE_SPACE;//gmpping
        }

        matchSize();

        nh_.param<bool>("resize_to_boundary", resize_to_boundary_, true);//for gmapping, it false
    ...
    void BoundedExploreLayer::matchSize(){
        Costmap2D* master = layered_costmap_->getCostmap();//此处得到static layer
        resizeMap(master->getSizeInCellsX(), master->getSizeInCellsY(), master->getResolution(),
                  master->getOriginX(), master->getOriginY());//初始化代价地图为默认值FREE_SPACE
    }
