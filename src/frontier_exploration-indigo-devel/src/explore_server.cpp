...
private:
    ...
    void executeCb(const frontier_exploration::ExploreTaskGoalConstPtr &goal)
    {
        ...
        explore_costmap_ros_->resetLayers();//引起代价地图跟新,调用updateBound,updateCost
        ...
        while(ros::ok() && as_.isActive()){
            ...
            tf::Stamped<tf::Pose> robot_pose;//pose=position + oritention
            explore_costmap_ros_->getRobotPose(robot_pose);

            //provide current robot pose to the frontier search service request
            tf::poseStampedTFToMsg(robot_pose,srv.request.start_pose);//将robot_pose(tf类型)转换为geometry_msgs::PoseStamped
    ...
    void preemptCb(){
        ...
        boost::unique_lock<boost::mutex> lock(move_client_lock_);//生命结束，自动解锁
        move_client_.cancelGoalsAtAndBeforeTime(ros::Time::now());
        ROS_WARN("Current exploration task cancelled");

        if(as_.isActive()){
            as_.setPreempted();//将当前目标状态置为抢占
        }
...
/**
 * @brief pub the clicked point to fake the frontier exploration.
 */
...
int main(int argc, char** argv)
{
    ros::init(argc, argv);
    ros::NodeHandle nh;
    ros::Publisher puber = nh.advertise<geometry_msgs::PointStamped>("clicked_point", 1);
    geometry_msgs::PointStamped p;
    p.header.frame_id = "map";
    p.point.z = 0.0;
    float tmp[5][2] = {{10.0, 10.0},
                       {-10.0, 10.0},
                       {-10.0, -10.0},
                       {10.0, -10.0},
                       {0.1, 0.0}}; //center
